...
export default function CatadorDashboard({ onLogout }: CatadorDashboardProps) {
  ...
  const handleSaveScore = async () => {
    ...
    try {
      ...
      if (existingScore) {
        ...
        const { error } = await supabase.from("puntuaciones_catadores").insert({
  ...
  return (
    <div className="min-h-screen bg-[#E6EBEE] p-4 md:p-8">
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-2xl font-bold text-gray-900">
                  {muestras.length - puntuaciones.size}
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="divide-y">
              {muestras.map((muestra) => {
                const {
                  ...
                  color,
                  bgColor,
                } = getMuestraStatus(Number(muestra.id));
                const existingScore = puntuaciones.get(Number(muestra.id));
                const isEditing =
                  editingScore?.muestra_id === Number(muestra.id);

                return (
                  <div
                    key={muestra.id}
                    className={`p-6 hover:bg-gray-50 transition-colors ${isEditing ? "bg-blue-50" : ""}`}
                  >
                          ...
                          <div className="mt-4 space-y-4 bg-white p-4 rounded-lg border-2 border-blue-200">
                            ...
                            <div className="flex gap-3">
                              ...
                              >
                                ...
                                {saving === editingScore.muestra_id
                                  ? "Guardando..."
                                  : "Guardar Puntuación"}
                              </button>
                              <button
