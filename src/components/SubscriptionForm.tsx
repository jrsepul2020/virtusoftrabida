...
// He añadido 'Trash2' al import ya que se usa en el componente
import { Building2, X as XIcon, Trash2 } from 'lucide-react'; 
...
export default function SubscriptionForm() {
    ...
    const createSamplesArray = async () => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (isManualInscription) {
          // Si el código ya está asignado, lo mantenemos; si es nuevo, generamos uno.
          let codeValue = samples[i]?.codigo;
          if (!codeValue) {
            const code = await generateUniqueManualCode();
            codeValue = code.toString();
          }
          sample.manual = true;
          sample.codigo = codeValue;
        }
        // Conservar datos si ya existían para evitar perder inputs al cambiar el total
        const existingSampleData = samples[i] || {};
        newSamples.push({ ...sample, ...existingSampleData });
      }
  ...
  const handleManualInscriptionToggle = async (checked: boolean) => {
    ...
    // Regenerar muestras con códigos si es necesario al activar
    if (checked) {
      const updatedSamples = await Promise.all(samples.map(async (sample) => {
        if (!sample.codigo || !sample.manual) {
          const code = await generateUniqueManualCode();
          return { ...sample, manual: true, codigo: code.toString() };
        }
        return { ...sample, manual: true };
      }));
      setSamples(updatedSamples);
    } else {
      // Limpiar códigos manuales al desactivar
      setSamples(samples.map(sample => ({ ...sample, manual: false, codigo: '' })));
    }
  };
    ...
    try {
      ...
      if (existingCompany) {
        ...
        if (authData.user) {
          ...
          // Usar un modal o un mensaje de éxito en la UI en lugar de alert()
          setSuccess(true);
        }
      }
      ...
      if (validSamples.length > 0) {
        ...
        try {
          // Asumo que VITE_SUPABASE_URL y VITE_SUPABASE_ANON_KEY están disponibles en el entorno de ejecución
          // En entornos de Canvas, estas variables deben estar predefinidas para la ejecución de funciones
  ...
  // APLICO EL FONDO GENERAL AQUÍ: bg-gray-100 y min-h-screen para cubrir toda la pantalla
  return (
    <div className="min-h-screen bg-white p-4 sm:p-8">
      <form onSubmit={handleSubmit} className="max-w-6xl mx-auto bg-white p-8">
        ...
        {success && (
          <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg mb-6">
            ¡Inscripción exitosa! Se ha enviado la información a su correo.
          </div>
        )}

        {loading && (
          <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-lg mb-6">
            Cargando... Por favor, espere mientras procesamos la inscripción.
          </div>
        )}

        <div className="mb-8">
          <h3 className="text-xl font-semibold text-gray-700 mb-4">Datos de la Empresa</h3>
          {/* Se ha añadido el fondo, padding, esquinas redondeadas y un borde para destacar la sección de campos. */}
          <div className="space-y-4 p-6 bg-virtus-fondos rounded-xl border border-gray-200"> 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                    ...
                    <label
                      ...
                      className={`flex items-center justify-center px-4 py-2 border-2 rounded-lg cursor-pointer transition-all ${
                        companyData.totalinscripciones === num
                          ? 'border-primary-600 bg-white text-primary-700 font-semibold'
                ...
                {companyData.totalinscripciones > 0 && (
                  <p className="block text-sm font-large text-gray-700 mb-1">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-4">
            {samples.map((sample, index) => (
              <div key={index} className="border border-gray-200 rounded-lg p-4 bg-virtus-fondos-2">
                <div className="mb-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    <div className="md:col-span-3">
                      <div className="bg-primary-50 border border-primary-200 rounded-lg p-3">
                        <label className="block text-sm font-medium text-primary-800 mb-1">
                          Código de Muestra Manual
                        </label>
                        <input
                          type="number"
                          value={sample.codigo}
                          onChange={(e) => updateSample(index, 'codigo', e.target.value)}
                          ...
                          placeholder="Código entre 1 y 999"
                          min="1"
                          max="999"
                          maxlength="3"
                        />
                        <p className="mt-1 text-xs text-primary-600">
                          Debe ser un código único entre 1 y 999.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {/* Resto de campos de muestra, que ya tienen un div con bg-gray-50 */}
                  
                  {/* Inicio del código que faltaba en el prompt original */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      País
                    </label>
                  ...
                  {/* Fin del código que faltaba en el prompt original */}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mt-8 pt-6 border-t border-gray-200 flex justify-end">
          <button
            type="submit"
            disabled={loading}
            className={`px-8 py-3 rounded-xl font-bold text-lg transition-all shadow-lg hover:shadow-xl ${
              loading
                ? 'bg-virtus-fondos text-gray-700 cursor-not-allowed'
                : 'bg-primary-600 text-white hover:bg-primary-700'
            }`}
          >
            {loading ? 'Procesando...' : 'Inscribir Muestras y Continuar al Pago'}
          </button>
        </div>
      </form>
    </div>
