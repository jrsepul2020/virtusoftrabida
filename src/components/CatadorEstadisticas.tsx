...
export default function CatadorEstadisticas({
  catadorId,
  mesaId,
}: CatadorEstadisticasProps) {
  ...
  const loadEstadisticas = async () => {
    try {
      ...
      const promedio =
        puntuacionesNumeros.length > 0
          ? puntuacionesNumeros.reduce((a, b) => a + b, 0) /
            puntuacionesNumeros.length
          : 0;

      // Desviación estándar
      const varianza =
        puntuacionesNumeros.length > 0
          ? puntuacionesNumeros.reduce(
              (sum, val) => sum + Math.pow(val - promedio, 2),
              0,
            ) / puntuacionesNumeros.length
          : 0;
      ...
      <div className="space-y-6 animate-pulse">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="bg-white rounded-lg p-6 h-32 border border-gray-200"
            ></div>
          ))}
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg p-6 border border-gray-200">
          ...
          <p className="text-3xl font-bold text-gray-900">
            {estadisticas.promedioPuntuacion}
          </p>
        ...
        <div className="bg-white rounded-lg p-6 border border-gray-200">
          <div className="flex items-center gap-3 mb-2">
            <div
              className={`w-10 h-10 rounded-lg flex items-center justify-center ${getConsistenciaColor(estadisticas.consistencia)}`}
            >
          ...
          <p className="text-xs text-gray-500 mt-1">
            σ = {estadisticas.desviacionEstandar}
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg p-6 border border-gray-200">
          ...
          <p className="text-3xl font-bold text-gray-900">
            {estadisticas.comparativaMesa.findIndex(
              (c) => c.catador === estadisticas.comparativaMesa[0]?.catador,
            ) + 1}
            °
          </p>
          <p className="text-xs text-gray-500 mt-1">
            de {estadisticas.comparativaMesa.length} catadores
          </p>
        </div>
      </div>
