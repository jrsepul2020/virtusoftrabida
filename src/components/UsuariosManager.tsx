...
export default function UsuariosManager() {
  ...
  const usuariosFiltrados = usuarios.filter((u) => {
    const filterNombre = filters.nombre || "";
    const filterEmail = filters.email || "";
    const filterPais = filters.pais || "";
    const filterCodigo = filters.codigo || "";

    const matchNombre = (u.nombre || "").toLowerCase().includes(filterNombre.toLowerCase());
    const matchEmail = (u.email || "").toLowerCase().includes(filterEmail.toLowerCase());
    const matchPais = (u.pais || "").toLowerCase().includes(filterPais.toLowerCase());
    ...
    const matchCodigoCatador = filters.codigocatador
      ...
      : true;
    const matchCodigo = (u.codigo || "").toLowerCase().includes(filterCodigo.toLowerCase());
