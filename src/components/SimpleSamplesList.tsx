...
export default function SimpleSamplesList({
...
}: SimpleSamplesListProps) {
  const [activeSubTab, setActiveSubTab] = useState<
    "gestion" | "etiquetado" | "categorias" | "chequeo"
  >("gestion");
  ...
  const fetchSamples = async () => {
    ...
    try {
      ...
      const samplesWithEmpresa =
        samplesData?.map((sample) => ({
          ...sample,
          empresa_nombre:
            sample.empresas?.name || sample.empresa || "Sin empresa",
  ...
  const handleDeleteSample = async (sample: Sample) => {
    if (
      !confirm(
        `¿Estás seguro de que deseas eliminar la muestra "${sample.nombre}"?`,
      )
    ) {
      return;
    }
  ...
  const handleExportToExcel = () => {
    ...
    const excelData = filteredSamples.map((sample) => ({
      ...
      País: sample.pais || "",
      "F. Inscripción": sample.created_at
    ...
    const categoriaTexto =
      selectedCategories.length > 0 ? `_${selectedCategories.join("_")}` : "";
  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-6 inline-flex rounded-lg bg-gray-50 p-1 shadow-sm">
        ...
        <button
          onClick={() => setActiveSubTab("etiquetado")}
          className={`px-4 py-2 rounded-lg font-semibold text-sm transition-all duration-200 ${
            activeSubTab === "etiquetado"
              ? "bg-red-700 text-white shadow-md"
              : "bg-transparent text-gray-600 hover:bg-white hover:text-gray-900"
          }`}
        >
        ...
        <button
          onClick={() => setActiveSubTab("chequeo")}
          className={`px-4 py-2 rounded-lg font-semibold text-sm transition-all duration-200 ${
            activeSubTab === "chequeo"
              ? "bg-red-700 text-white shadow-md"
              : "bg-transparent text-gray-600 hover:bg-white hover:text-gray-900"
          }`}
        >
                  ...
                  {filteredSamples
                    ...
                    >
                      <td className="px-2 py-1.5 whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          {sample.manual && (
                            <Hand className="w-3 h-3 text-red-600 flex-shrink-0" />
                          )}
                          {sample.categoriadecata === "vinos_tranquilos" && (
                            <Wine className="w-3 h-3 text-primary-600 flex-shrink-0" />
                          )}
                          {sample.categoriadecata ===
                            "generosos_espirituosos" && (
                            <Beer className="w-3 h-3 text-amber-600 flex-shrink-0" />
                          )}
                          {sample.categoriadecata === "aoves_cata" && (
                            <Droplet className="w-3 h-3 text-green-600 flex-shrink-0" />
                          )}
                          <span
                      ...
                      <td className="px-2 py-1.5 border-l border-gray-200">
                        <span
                          className={`text-xs ${sample.manual ? "text-red-700" : "text-gray-900"}`}
                        >
                      ...
                      <td className="px-2 py-1.5 text-center border-l border-gray-200">
                        {sample.empresa_pedido ? (
                          <span
                            className={`inline-flex items-center justify-center px-1.5 py-0.5 rounded font-semibold text-xs ${
                              sample.manual
                  ...
                  <div className="space-y-1 text-xs text-gray-600">
                    {sample.empresa_nombre && (
                      <div className="flex justify-between">
                        <span>Empresa:</span>
                        <span
                          className={`font-medium ${sample.manual ? "text-red-700" : "text-gray-900"}`}
                        >
                          {sample.empresa_nombre}
                        </span>
                      </div>
              ...
              <div className="bg-white rounded-xl max-w-4xl w-full p-6 max-h-[90vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-900 font-mono text-lg">
                        #{viewingSample.codigotexto || viewingSample.codigo}
                      </p>
                    </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {viewingSample.azucar !== null &&
                      viewingSample.azucar !== undefined && (
                        <div>
                          <label className="block text-sm font-medium text-gray-500">
                            Azúcar
                          </label>
                          <p className="text-gray-900">
                            {viewingSample.azucar} g/L
                          </p>
                        </div>
                      )}

                    {viewingSample.grado !== null &&
                      viewingSample.grado !== undefined && (
                        <div>
                          <label className="block text-sm font-medium text-gray-500">
                            Grado Alcohólico
                          </label>
