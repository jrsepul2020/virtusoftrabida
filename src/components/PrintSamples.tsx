...
export default function PrintSamples() {
  ...
  const [sortField, setSortField] = useState<"codigo" | "nombre" | "categoria">(
    "codigo",
  );
      ...
      <div className="no-print bg-white rounded-xl shadow-md p-4 sm:p-6 mb-6">
        ...
        <div className="mb-4 flex flex-col sm:flex-row gap-3 sm:gap-4">
          ...
          <div className="flex gap-2">
            ...
            >
              Nombre
              {sortField === "nombre" && <ArrowUpDown className="w-3 h-3" />}
            </button>
            <button
            ...
            >
              Categoría
              {sortField === "categoria" && <ArrowUpDown className="w-3 h-3" />}
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm text-gray-600">
            {selectedSamples.size} de {filteredSamples.length} muestras
            seleccionadas
          </span>
        </div>

        <div
          className={`border ${showGridLines ? "border-gray-200" : "border-transparent"} rounded-lg overflow-hidden max-h-96 overflow-y-auto`}
        >
          <table
            className={`min-w-full ${showGridLines ? "divide-y divide-gray-200" : ""}`}
          >
            <thead
              className={`${showGridLines ? "bg-gray-50" : ""} sticky top-0`}
            >
              <tr>
                <th
                  className={`${compactPrint ? "px-3 py-2" : "px-4 py-3"} text-left text-xs font-medium text-gray-500 uppercase tracking-wider`}
                >
                  <input
                    type="checkbox"
                    checked={
            ...
            <tbody
              className={`bg-white ${showGridLines ? "divide-y divide-gray-100" : ""}`}
            >
              {filteredSamples.map((sample) => (
                ...
                >
                  <td
                    className={`${compactPrint ? "px-3 py-2" : "px-4 py-3"} whitespace-nowrap`}
                  >
                    <input
                      type="checkbox"
                  ...
                  <td
                    className={`${compactPrint ? "px-3 py-2 text-xs" : "px-4 py-3 text-sm"} whitespace-nowrap font-bold text-gray-900`}
                  >
                    {sample.codigo}
                  </td>
                  <td
                    className={`${compactPrint ? "px-3 py-2 text-xs" : "px-4 py-3 text-sm"} text-gray-900`}
                  >
                    {sample.nombre}
                  </td>
                  <td
                    className={`${compactPrint ? "px-3 py-2 text-xs" : "px-4 py-3 text-sm"} whitespace-nowrap text-gray-700`}
                  >
                    {sample.categoria || "-"}
                  </td>
                  <td
                    className={`${compactPrint ? "px-3 py-2 text-xs" : "px-4 py-3 text-sm"} text-gray-700`}
                  >
                    {sample.empresa || "-"}
                  </td>
                </tr>
      ...
      <div className="print-only">
        <div className="print-header">
          ...
          <p className="print-total">
            Total de muestras: {selectedSamples.size}
          </p>
        </div>
