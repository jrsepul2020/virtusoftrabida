...
export async function checkDeviceAccess(
  ...
  try {
    ...
    if (!device) {
      ...
      const isFirstDevice =
        !countError && (!activeDevices || activeDevices.length === 0);
      ...
      if (isFirstDevice) {
        ...
        if (insertError) {
          ...
          return {
            allowed: false,
            reason: "Error al registrar dispositivo de administrador",
          };
        }
        ...
        if (existingUser) {
          ...
          if (userUpdateError) {
            console.warn(
              "Warning: Could not update user role to Administrador:",
              userUpdateError,
            );
          } else {
          ...
          if (userInsertError) {
            console.warn(
              "Warning: Could not create user as Administrador:",
              userInsertError,
            );
          } else {
        ...
        console.log(
          "✅ First admin device registered and activated:",
          newDevice.id,
        );
...
export async function loadUserRole(
  ...
  try {
    ...
    // 1. Get user data - try user_id first
    ...
    // 2. Fallback to id if not found by user_id
    if (!user && !userError) {
      console.log("ℹ️ User not found by user_id, trying by id:", userId);
